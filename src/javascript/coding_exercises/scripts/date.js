...
function findDuplicateTransactions(transactions = []) {
  ...
  const transactionsSortedByTime = (a, b) => new Date(a.time) - new Date(b.time)

  const diffAMinute = (start, end) => {
    const diff = new Date(start) - new Date(end)
    return diff / 60000
  }
    ...
    .map(element => {
      ...
      element.map((transaction, index) => {
        if (index === 0) {
          ...
          diffAMinute(transaction.time, element[index - 1].time) < 1 &&
          diffAMinute(transaction.time, element[index - 1].time > 0)
        ) {
