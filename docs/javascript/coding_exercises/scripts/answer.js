function findDuplicateTransactions(transactions = []) {
  ...
  const tempOutputArray = transactions
    .map(element => ({
      key: JSON.stringify([
        element.sourceAccount,
        element.targetAccount,
        element.amount,
        element.category,
      ]),
      epoch: Date.parse(element.time),
      element,
    }))
    .sort((a, b) => a.epoch - b.epoch)
    .reduce(
      ([acc, prev], curr) => {
        if (!prev || curr.key !== prev.key || curr.epoch - prev.epoch > 60000) {
          acc.push([])
        }
        acc[acc.length - 1].push(curr.element)
        return [acc, curr]
      },
      [[]]
    )[0]

  return tempOutputArray.filter(a => a.length > 1)
}
