...
const makeSentence = (person: { name: string; age: number }) =>
  `${person.name} is ${person.age} years old`

const peopleSentences = (peopleArr: { name: string; age: number }[]) =>
  peopleArr.map(makeSentence)

peopleSentences(people)

// Another map() example
export const valuesArr = [1, 2, 3, -4, 5]

// eslint-disable-next-line no-shadow
export const updateListMap = (valuesArr: number[]) => valuesArr.map(Math.abs)
...
function mapForEach(
  arr: string | any[],
  fn: { (item: string): number; (arg0: any): any }
) {
  const newArray = []
...
const lenArray: number[] = mapForEach(strArray, (item: string) => item.length)
...
/*
 Custom filter
 */
Array.prototype.myFilter = function myFilter(
  callback: { call: (arg0: any, arg1: any, arg2: number, arg3: any) => any },
  context: any
) {
  const arr = []
...
/*
 Custom reduce
 */
Array.prototype.myReduce = function myReduce(
  callback: {
    call: (
      arg0: undefined,
      arg1: any,
